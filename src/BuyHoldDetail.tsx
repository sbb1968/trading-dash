...
// BuyHoldDetail — den DETALJEREDE køb-og-hold-score for ÉN ticker, vist i sit eget
// vindue (åbnes ved dobbeltklik i Buy-and-Hold-scores eller Top-15). Flere kan være
// åbne samtidig. Selv-analyserer ved mount / når ticker skifter.
...
function Badge({ text, color }: { text: string; color: string }) {
  return (
    <span style={{ fontSize: 12, fontWeight: 700, color, border: `1px solid ${color}`, borderRadius: 4, padding: "1px 6px", whiteSpace: "nowrap", background: "var(--bg-overlay)" }}>{text}</span>
  );
}
...
function LayerCard({ title, layer }: { title: string; layer: Layer }) {
  const empty = !layer.groups || layer.groups.length === 0;
  return (
    <div style={{ flex: 1, minWidth: 0, background: "var(--bg-surface)", border: "1px solid var(--border-subtle)", borderRadius: 8, padding: 12 }}>
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginTop: 4 }}>
        {empty ? (
          <><span style={{ fontSize: 22, fontWeight: 800, color: "var(--text-muted)" }}>—</span><span style={{ fontSize: 12, color: "var(--text-muted)" }}>ingen data</span></>
        ) : (
          <><span style={{ fontSize: 22, fontWeight: 800, color: scoreColor(layer.score) }}>{signed(layer.score)}</span><Badge text={bandLabel(layer.band)} color={scoreColor(layer.score)} /></>
        )}
      </div>
...
function FlagStripe({ text }: { text: string }) {
  return (
    <div style={{ background: "var(--bg-overlay)", border: "1px solid var(--neutral)", borderRadius: 6, padding: "8px 12px", fontSize: 13, color: "var(--neutral)" }}>⚠ {text}</div>
  );
}

// ============================================================================
export function BuyHoldDetail({ ticker }: { ticker: string }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--bg-base)", color: "var(--text-primary)" }}>
      <div style={{ padding: "8px 12px", borderBottom: "1px solid var(--border-subtle)", display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
        <div style={{ flex: 1, fontSize: 14, fontWeight: 800, letterSpacing: "0.5px" }}>{(ticker || "").toUpperCase()}</div>
        <button onClick={analyze} disabled={loading}
          style={{ background: loading ? "var(--bg-elevated)" : "var(--accent)", color: loading ? "var(--text-muted)" : "#fff", border: "none", borderRadius: 4, padding: "6px 14px", fontSize: 13, fontWeight: 700, cursor: loading ? "default" : "pointer" }}>
          {loading ? "Analyserer..." : "Gen-analysér"}
        </button>
            ...
            {oe && (
              <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border-subtle)", borderRadius: 8, padding: 12, display: "flex", gap: 16, flexWrap: "wrap" }}>
                ...
                <div style={{ minWidth: 210, border: "1px solid var(--accent)", borderRadius: 8, padding: "10px 12px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                  <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>Owner-Earnings yield</div>
