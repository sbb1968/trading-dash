    ...
    <div style={{ flex: 1, minWidth: 0, background: "var(--bg-surface)", border: "1px solid var(--border-subtle)", borderRadius: 8, padding: 12 }}>
      ...
      <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        {layer.groups.length === 0 && (
          <span style={{ fontSize: 13, color: "var(--text-muted)" }}>{pending ? "Realtids-news/halts ikke aktiveret endnu." : "Ingen faktorer i dette lag."}</span>
        )}
      </div>
    </div>
  );
...
export function DaytradingDetail({ ticker }: { ticker: string }) {
  ...
  }, [ticker, timeframe, sr, pattern, candle]);

  useEffect(() => { analyze(false); /* eslint-disable-next-line */ }, [ticker, timeframe]);
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--bg-base)", color: "var(--text-primary)" }}>
      <div style={{ padding: "8px 12px", borderBottom: "1px solid var(--border-subtle)", display: "flex", flexDirection: "column", gap: 8, flexShrink: 0 }}>
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "var(--text-muted)", cursor: "pointer" }}>
          ...
          Inkluder manuelt chart-overlay (din egen chart-vurdering) — tryk Opdater
        </label>
