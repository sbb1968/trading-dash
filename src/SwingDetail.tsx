...
// SwingDetail — den DETALJEREDE swing-score for ÉN ticker, vist i sit eget vindue
// (åbnes ved dobbeltklik på en ticker i Swing trading-scores eller Top-15). Flere
// kan være åbne samtidig for forskellige tickers. Selv-analyserer ved mount.
...
// ============================================================================
export function SwingDetail({ ticker }: { ticker: string }) {
  ...
  const analyze = useCallback(async (withOverlay: boolean) => {
    const t = (ticker || "").trim().toUpperCase();
    if (!t) return;
    setLoading(true); setError(""); setData(null);
    try {
      const body: any = { ticker: t };
      if (withOverlay) { body.sr = sr; body.pattern = pattern; body.candle = candle; }
  ...
  }, [ticker, sr, pattern, candle]);

  // Analysér automatisk ved åbning / når ticker skifter (uden overlay).
  useEffect(() => { analyze(false); /* eslint-disable-next-line */ }, [ticker]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--bg-base)", color: "var(--text-primary)" }}>
      {/* Kontrol-bar: overlay + PDF + gen-analysér */}
      <div style={{ padding: "8px 12px", borderBottom: "1px solid var(--border-subtle)", display: "flex", flexDirection: "column", gap: 8, flexShrink: 0 }}>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <div style={{ flex: 1, fontSize: 14, fontWeight: 800, letterSpacing: "0.5px" }}>{(ticker || "").toUpperCase()}</div>
          <button onClick={() => analyze(useOverlay)} disabled={loading}
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "var(--text-muted)", cursor: "pointer" }}>
          ...
          Inkluder manuelt chart-overlay (din egen chart-vurdering) — tryk Gen-analysér
        </label>
